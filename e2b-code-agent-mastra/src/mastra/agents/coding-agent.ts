...
- \`killSandbox\` - Stop and release a sandbox when the task is complete
...
`,
  model: llmProvider(process.env.MODEL || 'gpt-4o'),
  tools: {
    createSandbox,
    killSandbox,
    runCode,
  ...
  memory: new Memory({
    ...
    embedder: fastembed,
    vector: new LibSQLVector({ id: 'agent-vector', url: 'file:../../mastra.db' }),
  }),
